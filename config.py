...
COMMAND_USAGE_HELP = {
    ...
    "addcase": "`.addcase [ID нарушителя] [ID модератора] [timestamp] [тип] [длительность (если мьют)] [причина]` - Перенести старое дело в базу.",
...
HELP_CATEGORIES = {
    ...
    "staff": {
        ...
        "commands": [
            ...
            "> `.warn` - *Выдать варн участнику.*",
            "> `.delwarn` - *Удалить варн участника.*",
            "> `.mute` - *Выдать мьют (тайм-аут) участнику.*",
            "> `.unmute` - *Снять мьют с участника.*",
            "> `.ban` - *Забанить участника навсегда.*",
            "> `.unban` - *Разбанить участника по ID.*",
            "> `.modlogs` - *Просмотреть список дел участника.*",
            "> `.modstats` - *Просмотреть статистику наказаний.*",
            "> `.moderations` - *Просмотреть наказания, выданные модератором.*",
            "> `.delcase` - *Удалить дело по его номеру.*",
            "> `.addcase` - *Импортировать старое дело из прошлых логов.*",
        ]
    }
}
