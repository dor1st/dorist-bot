...
def parse_duration(value: str) -> timedelta | None:
    ...
    30m, 2h, 7d, 2w, 1mo / 1month / 1months.
...
def build_giveaway_embed(
    *,
    ...
    if ended:
        embed.description = "~~Розыгрыш завершён~~"
    else:
        embed.description = (
            "Поставьте реакцию 🎉 под этим сообщением, "
    ...
    if min_messages > 0:
        requirements.append(
            f"• Минимум сообщений: **{min_messages}**"
        )

    if min_invites > 0:
        requirements.append(
            f"• Минимум приглашений: **{min_invites}**"
        )
    ...
    if required_roles:
        ...
        requirements.append(
            f"• Роли ({mode_text}): "
            f"{role_mentions(host.guild, required_roles)}"
        )

    embed.add_field(
        name="📋 Требования",
        value="\n".join(requirements) if requirements else "Нет",
        inline=False,
    )
    ...
    if ended:
        if winner_ids:
            ...
            embed.add_field(
                name="🎊 Победители",
                value="Подходящих участников не найдено.",
                inline=False,
            )
...
class GiveawaySetupView(discord.ui.View):
    ...
    async def interaction_check(
        ...
        if interaction.user.id != self.ctx.author.id:
            await interaction.response.send_message(
                "Только автор создания розыгрыша "
                "может менять его настройки.",
                ephemeral=True,
            )
            return False
    ...
    def setup_embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"**Количество победителей:** "
                f"{self.winners_count}\n"
        ...
        embed.add_field(
            ...
            value=(
                ...
                f"**Роли:** "
                f"{role_mentions(self.guild, self.required_roles)}"
            ),
            inline=False,
        )
        ...
        if self.bonus_roles:
            bonus_text = "\n".join(
                f"• {role_mentions(self.guild, [role_id])} "
                f"— **+{entries}**"
                for role_id, entries
                in self.bonus_roles.items()
            )
        else:
    ...
    async def bonus_button(
        ...
        await interaction.response.send_message(
            "Выберите роль и укажите количество "
    ...
    async def create_button(
        ...
        await interaction.followup.send(
            f"Розыгрыш создан: {message.jump_url}",
            ephemeral=True,
        )
...
class RoleModeSelect(discord.ui.Select):
    ...
    async def callback(
        ...
        await interaction.response.edit_message(
            content="Режим ролей обновлён. "
            "Теперь выберите роли ниже.",
            view=self.view,
        )
...
class RequirementsModal(
    ...
    async def on_submit(
        ...
        try:
            messages = int(
                str(self.min_messages.value).strip()
            )
            invites = int(
                str(self.min_invites.value).strip()
            )
...
class BonusRoleSelect(discord.ui.RoleSelect):
    def __init__(self, setup: GiveawaySetupView):
        ...
        super().__init__(
            placeholder=(
                "Выберите роль для дополнительного шанса"
            ),
            min_values=1,
            max_values=1,
            row=0,
...
class BonusEntriesModal(
    ...
    async def on_submit(
        ...
        try:
            ...
            await interaction.response.send_message(
                "Количество дополнительных шансов "
        ...
        await interaction.response.send_message(
            f"Для роли <@&{self.role_id}> установлено "
            f"**+{value}** шансов.",
            ephemeral=True,
        )
...
class GiveawayCog(commands.Cog):
    ...
    async def _get_giveaway_message(
        self,
        doc: dict,
    ):
    ...
    def _is_eligible(
        ...
        if user_doc.get(
            "messages_count",
            0,
        ) < doc.get("min_messages", 0):
            return False

        total_invites = (
            user_doc.get("real_invites", 0)
            + user_doc.get("bonus_invites", 0)
        )

        if total_invites < doc.get(
            "min_invites",
            0,
        ):
            return False
    ...
    def _weight(
        ...
        for role_id, entries in doc.get(
            "bonus_roles",
            {},
        ).items():
    ...
    async def _update_participant_count(
        ...
        if (
            payload.emoji.name != GIVEAWAY_EMOJI
            or (
                bot_user
                and payload.user_id == bot_user.id
            )
        ):
            return
        ...
        embed = build_giveaway_embed(
            ...
            participant_count=max(
                0,
                doc.get("participant_count", 0),
            ),
            role_mode=doc.get("role_mode"),
            required_roles=doc.get(
                "required_roles",
                [],
            ),
            min_messages=doc.get(
                "min_messages",
                0,
            ),
            min_invites=doc.get(
    ...
    async def on_raw_reaction_add(
        ...
        await self._update_participant_count(
            payload,
            +1,
        )
    ...
    async def finish_giveaway(
        ...
        *,
        ...
        for user in users:
            ...
            if member and self._is_eligible(
                member,
                doc,
            ):
                eligible.append(
        ...
        # Взвешенный случайный выбор без повторений.
    ...
    async def finish_loop(self):
        ...
        for doc in docs:
            try:
                ...
                # Ошибка одного розыгрыша не должна
                # останавливать автоматическое завершение остальных.
                continue
    ...
    async def giveaway_create(
        ...
        *args: str,
        ...
        if len(args) < 4:
            await ctx.send(
                embed=make_error_embed(
                    ...
                    (
                        "Использование:\n"
                        "`.giveaway create [приз] "
                        "[длительность] [победители] "
                        "[время на получение]`\n\n"
                        "Пример:\n"
                        "`.giveaway create 1000 Robux "
                        "7d 1 24h`"
                    ),
                )
            )
        ...
        # Последние 3 аргумента фиксированы,
        # всё до них считается названием приза.
        prize = " ".join(
            args[:-3]
        ).strip()
    ...
    async def giveaway_end(
        ...
        await ctx.send(
            embed=make_status_embed(
                "Розыгрыш завершён",
                (
                    f"Розыгрыш `{message_id_int}` "
                    "был завершён досрочно."
                ),
                "success",
            )
        )
    ...
    async def giveaway_delete(
        ...
        message = await self._get_giveaway_message(
            doc
        )
